...
type ImageCreatorWorkItemType = 'text' | 'image';
...
export const ImageCreatorWorkArea = ({
    ...
    const [newImageDialogOpen, setNewImageDialogOpen] = useState<boolean>(false);
    ...
    return <React.Fragment>
        <Box sx={{ backgroundColor: 'rgba(255,238,81,.05)', display: 'flex', justifyContent: 'center', padding: '4px' }}>
            {
                selectedWorkItem && <Typography>Tap text to drop it.</Typography>
            }
            {
                workItems.length > 0 && !selectedWorkItem && <Typography>Tap text to move it.</Typography>
            }
            {
                workItems.length > 0 &&
                workItems.some(({content}) => content === DEFAULT_ITEM_TEXT) &&
                <Typography><Link to="#text-options">Click for text options.</Link></Typography>
            }
        ...
        </Box>
        {
            workItems.length > 0 && <Box id="text-options" className="workArea-itemsOptions">
                {
                    workItems.map((item: ImageCreatorWorkItem) => <ImageCreatorTextEdit
                        workItem={item}
        ...
        >
            <MenuItem onClick={() => {
                    ...
                    addOrEditWorkItem(createNewWorkItem(position));
                    handleMenuClose();
                }}
            >
                Add text</MenuItem>
            ...
            {/*<MenuItem onClick={() => {*/}
                {/*setNewImageDialogOpen(true); handleMenuClose();*/}
            {/*}}*/}
            {/*>{ hasItemsOfType('image') ? 'Edit' : 'Add' } image</MenuItem>*/}
        ...
        <NewImageDialog
            ...
            label={'foreground'}
            inputKey={imageItem?.inputKey || getRandomInputKey()}
            onImageUpload={handleImageChange}
            onImageReset={(_event: any) => {
                handleItemsReset('image');
                onImageReset();
